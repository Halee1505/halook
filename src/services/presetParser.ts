...
export const fetchPresetPayload = async (
  fileUrl?: string
): Promise<string | null> => {
  if (!fileUrl) {
    return null;
...
const parseXmpPreset = (payload: string): PresetAdjustment | null => {
  ...
  COLOR_CHANNELS.forEach((channel) => {
    ...
    if (attributes[saturationKey] !== undefined) {
      preset.colorMix.saturation[channel] = parseNumber(
        attributes[saturationKey]
      );
      hasData = true;
    }
    ...
    if (attributes[luminanceKey] !== undefined) {
      preset.colorMix.luminance[channel] = parseNumber(
        attributes[luminanceKey]
      );
      hasData = true;
    }
  });
...
export const parsePresetPayload = (
  payload: string,
  fallbackSeed = ""
): PresetAdjustment => {
...
export const loadPresetAdjustments = async (
  fileUrl?: string,
  fallbackSeed = ""
) => {
